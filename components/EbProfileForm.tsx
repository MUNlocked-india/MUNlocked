...
export default function EbProfileForm({ error }: { error?: string }) {
  ...
  function toggleExpertise(committee: string) {
    setExpertise((current) => current.includes(committee) ? current.filter((item) => item !== committee) : [...current, committee]);
  }
  ...
  return <form action={submitEbApplication} encType="multipart/form-data" className="eb-profile-form">
    ...
    <section className="eb-form-section"><div className="eb-section-label"><span>02</span><div><h2>Your MUN record</h2><p>Use the number of conferences, not a written description.</p></div></div><div className="eb-form-grid"><label className="eb-field"><span>Executive Board experience</span><input name="eb_experience" type="number" required min="0" step="1" inputMode="numeric" placeholder="e.g. 4" /><small>Number of conferences where you served on the dais.</small></label><label className="eb-field"><span>Delegate experience</span><input name="delegate_experience" type="number" required min="0" step="1" inputMode="numeric" placeholder="e.g. 12" /><small>Number of conferences attended as a delegate.</small></label><label className="eb-field eb-field-wide"><span>Previous conferences <em>Optional</em></span><textarea name="previous_conferences" rows={2} placeholder="Example: Aethris MUN 2025 — Vice Chair, UNSC" /></label></div></section>

    <section className="eb-form-section"><div className="eb-section-label"><span>03</span><div><h2>Where you work best</h2><p>Tick every committee family you can chair confidently.</p></div></div><fieldset className="eb-expertise-checklist"><legend>Areas of expertise <small>Choose at least one</small></legend><div>{COMMITTEE_EXPERTISE.map((committee) => <label key={committee} className={expertise.includes(committee) ? "is-selected" : ""}><input name="areas_of_expertise" type="checkbox" value={committee} checked={expertise.includes(committee)} onChange={() => toggleExpertise(committee)} /><span>{committee}</span></label>)}</div></fieldset></section>
