...
export default function AboutPage() {
  return <main style={{ minHeight: "100vh", padding: "64px 24px 110px" }}><style>{`@keyframes drive{0%,100%{transform:translateX(0)}50%{transform:translateX(34px)}}@keyframes rise{from{opacity:0;transform:translateY(18px)}to{opacity:1;transform:translateY(0)}}.race-car{animation:drive 2.8s ease-in-out infinite}.race-station{animation:rise .55s both}`}</style><div style={{ maxWidth: 1100, margin: "0 auto" }}>
    ...
    <div style={{ borderTop: "2px dashed rgba(234,217,222,.25)", paddingTop: 30 }}><div className="race-car" style={{ fontSize: 30, marginTop: -54, marginBottom: 20 }}>🏎️</div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(185px,1fr))", gap: 16 }}>{STATIONS.map(([number, title, copy], index) => <section key={number} className="race-station" style={{ animationDelay: `${index * 120}ms`, background: "rgba(12,10,17,.78)", border: "1px solid rgba(234,217,222,.15)", borderRadius: 14, padding: 20, minHeight: 210 }}><div className="mono" style={{ color: "var(--brass)", fontSize: 11, letterSpacing: 2 }}>{number} · STATION</div><h2 style={{ fontFamily: "Georgia,serif", fontSize: 22, margin: "26px 0 12px" }}>{title}</h2><p style={{ fontSize: 13.5, lineHeight: 1.65, color: "rgba(234,217,222,.64)" }}>{copy}</p></section>)}</div></div>
  </div></main>;
}
