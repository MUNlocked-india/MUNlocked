...
export default async function InboxPage({ searchParams }: { searchParams: Promise<{ eb?: string; sent?: string; error?: string }> }) {
  ...
  return <div style={{ minHeight: "100vh", padding: "56px 24px" }}><div style={{ maxWidth: 900, margin: "0 auto" }}>
    ...
    <div style={{ display:"grid", gap:10 }}>{conversations?.length ? conversations.map(c => <div key={c.id} style={{ border:"1px solid rgba(234,217,222,.14)", padding:16, borderRadius:8 }}><b>{c.subject}</b><div className="mono" style={{ fontSize:11, opacity:.55, marginTop:6 }}>Conversation active · {new Date(c.created_at).toLocaleDateString()}</div></div>) : <p style={{ opacity:.65 }}>No conversations yet. <Link href="/hire-eb">Browse verified EBs →</Link></p>}</div>
  </div></div>;
}
