...
- **`/research`** is still a "coming next" stub page — upload/verification/watermarking is the next subsystem to build, same Supabase Storage + RLS approval pattern as everything above. Once it exists, a "new research submitted" admin notification should be added alongside the conference/EB ones, following the exact same `notifyAdmin()` pattern.
